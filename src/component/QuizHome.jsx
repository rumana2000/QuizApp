    ...
    <>
      <div className="main">
        <div className="container mx-auto">
          {quizList.length > 0 &&
            (
              <InfiniteScroll dataLength={quizList.length} hasMore={hasMore} loader={<>Loading...</>} next={() => setPage(page + 8)}>
                <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-4 gap-4  place-items-center">
                  {quizList.map((quizCategory, key) => (quizCategory.noq > 0 ? <NavLink key={key} to={`quiz/${quizCategory.quizCategoryId}`}><QuizCard quizData={quizCategory} /></NavLink> : <QuizCard quizData={quizCategory} />))}
                </div>
              </InfiniteScroll>
            )
          }
